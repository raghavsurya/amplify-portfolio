...
const App = () => {
  ...
  async function fetchAppointments() {
    try {
      const appointmentData = await API.graphql(graphqlOperation(listAppointments))
      const appointments = appointmentData.data.listAppointments.items
      setAppointments(appointments)
    } catch (err) { console.log('error fetching appointments') }
  }
  ...
  return (
    <div style={styles.container}>
      ...
      />
      <button style={styles.button} onClick={addAppointment}>Create Appointment</button>
      {
